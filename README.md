...
## 기능 목록
- [x] 시작 메시지 출력 `점심 메뉴 추천을 시작합니다.`
- [x] 코치 이름 입력 안내 `코치의 이름을 입력해 주세요. (, 로 구분)`
- [x] 코치 이름 입력
  - [x] 이름은 최소 2글자 최대 4글자
  - [x] 인원은 2명에서 5명
- [x] 각 코치가 못 먹는 메뉴 안내 및 입력 `토미(이)가 못 먹는 메뉴를 입력해 주세요.`
  - [x] 최소 0개(빈 값으로 입력) 최대 2개 메뉴
- [ ] 메뉴 추천
  - `Randoms.pickNumberInRange()` 사용
    - [x] 1이면 일식, 2면 한식, 3이면 중식, 4면 아시안, 5면 양식을 추천
    - [x] 추천할 수 없으면 다시
  - `Randoms.shuffle()`
    - [x] 메뉴에서 섞은 후 첫 번째 값 사용
    - [x] 메뉴는 List<String> 형태로 준비
    - [x] 추천할 수 없는 메뉴는 다시 섞은 후 첫 번째 값 사용
- [x] 메뉴 추천 결과
  - [x] 같은 카테고리 최대 2회
  - [x] 중복 메뉴 x
- [x] 추천 완료 메시지 출력 `추천을 완료했습니다.`
- [x] 에러 메시지 `[ERROR]`로 시작하는 메시지 출력 후 그 부분부터 다시 입력